...
class JDF {
    ...
    public  function convert_to_persian($date) {
        if (is_null($date) || empty($date)) {
            return $date;
        }
        $dateArray = explode('-', $date);
        $persianDate = $this->gregorian_to_jalali($dateArray[0], $dateArray[1], $dateArray[2]);
    ...
    public  function convert_to_gregorian($date) {
        if (is_null($date) || empty($date)) {
            return $date;
        }
        $dateArray = explode('/', $date);
        $persianDate = $this->jalali_to_gregorian($dateArray[0], $dateArray[1], $dateArray[2]);

        return implode('-', $persianDate);
    }
    
    public  function gregorian_to_timestamp($string){
        $jalali_array = explode("/", $string);
        $gregorian_string = $this->jalali_to_gregorian(current($jalali_array), next($jalali_array), next($jalali_array),"-");
